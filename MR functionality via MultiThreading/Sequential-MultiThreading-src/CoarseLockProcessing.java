...
public class CoarseLockProcessing {
	...
	public void CoarseLockExecution(CentralData c) throws Exception {

		final int totalCores 		= 8;	// threads to be spawned
		final String opFileName 	= "output/CoarseLockThreadOP.txt";
		...
		for (int iteration = 0; iteration < 10; iteration++) {
			...
			ArrayList<CoarseLock_ProcessorThread> pThreads = new ArrayList<CoarseLock_ProcessorThread>();
			...
			for (int i = 0; i < totalCores; i++) {
				...
				pThreads.add(new CoarseLock_ProcessorThread(startPos, endPos, c));
			}
