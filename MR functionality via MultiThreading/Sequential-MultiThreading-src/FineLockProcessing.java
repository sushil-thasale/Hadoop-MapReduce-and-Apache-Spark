...
public class FineLockProcessing {
	...
	public void FineLockExecution(CentralData c) throws Exception {

		final int totalCores 		= 8;	// threads to be spawned
		final String opFileName 	= "output/FineLockThreadOP.txt";
		...
		for (int iteration = 0; iteration < 10; iteration++) {
			...
			ArrayList<FineLock_ProcessorThread> pThreads = new ArrayList<FineLock_ProcessorThread>();
			...
			for (int i = 0; i < totalCores; i++) {
				...
				pThreads.add(new FineLock_ProcessorThread(startPos, endPos, c));
			}
