...
public class NoSharingProcessing {
	...
	public void NoSharingExecution(CentralData c) throws Exception {

		final int totalCores 		= 8;	// threads to be spawned
		final String opFileName 	= "output/NoSharing.txt";
		...
		for (int iteration = 0; iteration < 10; iteration++) {
			...
			// clears hashmap values for fresh iteration
			c.clearStationMap();
			...
			ArrayList<NoSharing_ProcessorThread> pThreads = new ArrayList<NoSharing_ProcessorThread>();
			...
			for (int i = 0; i < totalCores; i++) {
				...
				pThreads.add(new NoSharing_ProcessorThread(startPos, endPos, c));
			}
		...
		// clears hashmap values for next program execution
		c.clearStationMap();
	}
